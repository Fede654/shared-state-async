...
def _provenance(binary):
    """Delegates to experiments/provenance.py — the single implementation.

    This used to be a parallel copy, and it carried the same
    false-attribution bug: it recorded the CURRENT checkout as the
    binary's source, which is only true immediately after a rebuild.
    The shared version records `checkout_at_measurement` separately and
    reports the build revision only from a stamp written at build time.
    """
    sys.path.insert(0, os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "experiments"))
    import provenance
    return provenance.collect(binary, __file__)
